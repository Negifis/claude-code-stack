...
editing session as no evidence that the new prompt works. A new file waits for the next turn:
on 2026-09-26, 2.1.274 answered `Agent type … not found` for four profiles created in
...
A hook imports its sibling modules (`code_work_gate_common`, `continuity_common`,
`hygiene_common`) from its own folder, which Python puts first on `sys.path` when it runs a
script. A registration therefore never adds `-I` or `-P` and never sets `PYTHONSAFEPATH`: each
leaves that folder out, and `test_gate.py` asserts that too. Code that imports a hook as a
module puts the hooks folder on `sys.path` first: the suites do so by running from that folder,
and their `python -c` probes insert it; loading a hook by file path through `importlib` adds
nothing to `sys.path`.
...
Simplify lanes below XHIGH, all Sonnet, medium, `maxTurns: 40`: `agents/simplify-reviewer.md` covers reuse,
