...
| `hooks/gate_inbox.py` | CLI + `SessionStart` (digest) | Anomaly inbox `state/gate-anomalies.jsonl`: `report` files an agent's disagreement with a block (with the marker, the state, the session's ledger tail and the Stop hook's own view of the transcript); `scan` derives anomalies from the ledger by fixed rules; `list`/`show`/`ack` triage; `digest` injects the unresolved ones into a session started from `~/.claude`. |
