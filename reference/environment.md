...
| Piece | Event | Role |
|---|---|---|
| `hooks/code_work_gate_mark.py` | `PreToolUse`/`PostToolUse`/`PostToolUseFailure` on edits and shell | Marks the candidate (paths, class, risk floor); on `PostToolUse` injects one line when a candidate opens or its floor rises, and records a Codex outage from a finished `codex exec` call. A verdict expires only through a write-capable shell command (interpreter, redirect, heredoc, mutating git, `cp`/`sed -i`/…) whose effect the pre-command snapshot could not resolve; read-only pipelines never expire it. |
